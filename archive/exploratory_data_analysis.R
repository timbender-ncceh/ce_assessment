...
janitor::clean_names(ce2) %>%
  select(., 
         client_id, race, question, response, order_vuln) %>%
  group_by(client_id, race) %>%
  summarise(t_ov = sum(order_vuln)) %>%
  ggplot(data = ., 
         aes(x = race, y = t_ov, group = race)) + 
  geom_boxplot() +
  labs(title = "100% Survey Pop", 
       subtitle = "Race by unweighted summed order_vuln scores by client")+
  scale_y_continuous(limits = c(15, 40))+
  scale_x_discrete(limits = c("Asian", "Black", 
                              "Multiple Races", 
                              "Native American", 
                              "White", NA))
...
janitor::clean_names(ce2) %>%
  ...
  ggplot(data = ., 
         aes(x = race, y = t_ov, group = race)) + 
  geom_boxplot() +
  labs(title = "Top 20% of Survey Pop", 
       subtitle = "Race by unweighted summed order_vuln scores by client")+
 # scale_y_continuous(limits = c(15, 40))+
  scale_y_continuous(limits = c(15, 40))+
  scale_x_discrete(limits = c("Asian", "Black", 
...
ggplot() + 
  geom_density(aes(x = rbinom(10000, 9, 0.5)), adjust = 0.1)+
  scale_x_continuous(limits = c(0,9), breaks = seq(0,9,by=1))

ggplot() + 
  geom_line(aes(y=dbinom(1:10, 10, 0.5), 
                x = 1:10))+
